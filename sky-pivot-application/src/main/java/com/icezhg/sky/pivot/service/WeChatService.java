...
import com.icezhg.sky.pivot.exception.WeChatApiException;
...
public class WeChatService {
    ...
    public String getMiniAppOpenId(String code) {
        try {
            ...
            if (json.has("errcode") && json.get("errcode").asInt() != 0) {
                ...
                throw new WeChatApiException("WeChat login failed: " + errmsg);
            }

            if (!json.has("openid")) {
                throw new WeChatApiException("WeChat login failed: no openid in response");
            }
            ...
            throw new WeChatApiException("Failed to communicate with WeChat API", e);
        }
    }
    ...
    public String getPcOpenId(String code) {
        try {
            ...
            if (json.has("errcode") && json.get("errcode").asInt() != 0) {
                String errmsg = json.has("errmsg") ? json.get("errmsg").asText() : "Unknown error";
                throw new WeChatApiException("WeChat PC login failed: " + errmsg);
            }

            if (!json.has("openid")) {
                throw new WeChatApiException("WeChat PC login failed: no openid in response");
            }
